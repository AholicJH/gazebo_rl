...
def load_controller_srv(controller_name, ns=None, max_retries=5) -> bool:
    ...
    try: 
        for ii in range(max_retries):
            ...
            # rospy.loginfo("打印数据"+str(resp1.ok))
            if resp1.ok:
                return True
...
def switch_controllers_srv( start_controllers, stop_controllers, ns=None, 
                            strictness=1, start_asap=True, timeout=6.0, max_retries=5) -> bool:
    """
    ...
    # rospy.loginfo(start_controllers)
    ...
    try: 
        for ii in range(max_retries):
            ...
            # rospy.loginfo(srv_request)
            resp1 = client_srv(srv_request)
            # rospy.loginfo(f"Response: {resp1}")
            if resp1.ok:
                return True
