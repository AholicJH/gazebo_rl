...
class PavsReachAvoidEnv(RobotBasicEnv):
    ...
    def __init__(self):
        ...
        world_path = pkg_path +'/worlds/simple_world.world'
        world_pkg = 'robot_gazebo'
        world_filename = 'simple_world.world'
    ...
    def reset(self):
        ...
        self.target_position = np.array([2.0, -6.0, 0.014020], dtype=np.float32)
        self.car_velocity = np.array([0.0, 0.0], dtype=np.float32)
        self.laser_scan = np.zeros(60)  # 或者根据需要重新生成雷达数据
    ...
    def step(self, action):
        ...
        info['start_position'] = self.current_position
