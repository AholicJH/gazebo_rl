...
class PavsReachAvoidEnv(RobotBasicEnv):
    ...
    def __init__(self):
        ...
        定义状态空间和动作空间
        '''
        low = np.full((66,), 0.0, dtype=np.float32)
        ...
        self.target_position = np.array([4.000000, 0.000000, 0.000131], dtype=np.float32)  # 目标位置
    ...
    def _get_reward(self):
        ...
        reward_goal = dist_prev - dist_curr  # goal reward. close to goal, reward_goal is larger, else negative.
        reward += reward_goal
        # # 碰撞惩罚
        ...
        # 终止条件额外奖励
        if dist_curr < 0.3:  # 假设0.2米为成功到达目标的阈值
            reward += 1  # 给予一个额外的正奖励
    ...
    def _check_if_done(self):
        ...
        检查是否完成
        '''
        done, is_reach, is_collision, is_stepout = False, False, False, False
        ...
        done = is_reach or is_collision or is_stepout
    ...
    def laser_scan_callback(self, msg):
        ...
        # self.laser_scan = msg
        # 处理雷达数据，获取有效距离数据
        # self.print_once = True  # 初始时允许打印
        ranges = np.array(msg.ranges)
        ranges[ranges == float('inf')] = msg.range_max  # 将无穷远值替换为最大测量距离
        self.laser_scan = ranges
        # if self.current_step==1 and self.print_once:
        #     rospy.loginfo("lidar data"+str(self.laser_scan))
        #     self.print_once = False  # 打印一次后禁止再次打印
